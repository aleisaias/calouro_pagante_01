...
# Streamlit + Plotly • Aba 1: 5 dispersões (DELTAS) •
# Aba 2: 5 dispersões (%) + Heatmap Pearson + Spearman + Barras (IC95%)
# ================================================================
...
from scipy.stats import pearsonr, norm  # para IC95%
...
st.caption("Aba 1: DELTAS #2 (pp). Aba 2: Percentuais (%) + Heatmaps + Barras (IC95%). Cores = Δ % AUM.")

tab_deltas, tab_perc = st.tabs(["DELTAS #2 (5 dispersões)", "Percentuais: 5 dispersões + Heatmaps + Barras"])
...
st.markdown("---")
st.caption("Observação: Quadrantes em 0 (linhas tracejadas). Escala de cores: Viridis invertida (Δ % AUM).")
